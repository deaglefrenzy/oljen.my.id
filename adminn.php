...
if ($action == "editdistance") {
    ...
    $idorg = $_POST['idorg'];
    $wktin = $_POST['wktin'];
    mysqli_query($conn, "UPDATE rides SET distance='$distancebaru' WHERE id='$idrides'");
    $e = mysqli_fetch_array(mysqli_query($conn, "SELECT sum(distance) as tot FROM rides WHERE orang_id='$idorg'"));
    $jauh = $e['tot'];
    if ($jauh >= 1000) {
        mysqli_query($conn, "UPDATE orang SET finish='$wktin' WHERE id='$idorg'");
    }
}
if ($action == "orgbaru") {
    $namaorg = $_POST['namaorg'];
    $profile = $_POST['profile'];
    mysqli_query($conn, "INSERT INTO orang SET name='$namaorg', profile='$profile'") or die(mysqli_error($conn));
    pesan("Orang Baru!");
}
if ($action == "hapusrides") {
    $idrides = $_POST['idrides'];
    mysqli_query($conn, "DELETE FROM rides WHERE id='$idrides'");
    pesan("Ride Terhapus");
}
if ($action == "gantitgl") {
    $idrides = $_POST['idrides'];
    ...
    mysqli_query($conn, "UPDATE rides SET input_time='$gantinya' WHERE id='$idrides'");
...
if ($action == "isipesan") {
    ...
    mysqli_query($conn, "UPDATE rides SET pesan='$pesanbaru' WHERE id='$idrides'");
...
<body>
    ...
    ?>
        ...
        ?>
        <div>
            <div id="output-<?= $oljen ?>"></div>
            <form class="ajax"
                data-id="<?= $oljen ?>"
                data-handler="process.php">
                <input type="text" id="input-<?= $oljen ?>" placeholder="...">
                <button type="submit">Add Ride</button>
            </form>
        </div>
        <!--
        <div class="comment-section">
            <h2 class="section-title">Article #2 Comments</h2>

            <div id="comments-2" class="comments-container">
            </div>

            <form id="commentForm-2" class="comment-form"
                data-section-id="2"
                data-save-handler="save_comment.php"
                data-load-handler="get_comments.php">
                <textarea id="commentText-2" placeholder="Write your comment for Article #2 here..." rows="4"></textarea>
        ...
        <?php
        $count = 0;
        $q = mysqli_query($conn, "SELECT * FROM rides WHERE input_time LIKE '$date%' ORDER BY id DESC");
        while ($qq = mysqli_fetch_array($q)) {
            $count++;
            $idrides = $qq['id'];
            $idorang = $qq['orang_id'];
            $pesan = $qq['pesan'];
            $w = mysqli_fetch_array(mysqli_query($conn, "SELECT name FROM orang WHERE id='$idorang'"));
            $e = mysqli_fetch_array(mysqli_query($conn, "SELECT sum(distance) as tot FROM rides WHERE orang_id='$idorang' AND id<'$idrides'"));
        ?>
                ...
                <tr align="center">
                    ...
                    <form action="adminn.php?date=<?= $date ?>" method="post">
                        ...
                        <td rowspan="2" width="15%" <?php if ($qq['distance'] == 0) echo "style='background:yellow;'"; ?>>
                            <input type="tel" name="distancebaru" size="2" value="<?php echo $qq['distance']; ?>" id="inputSelect">
                        </td>
                    </form>
                </tr>
                <tr>
